...
Читать ленту можно **без регистрации** (всегда). **Аккаунт** — опционально: регистрация и вход на `/account`; после регистрации роль `reader`, права модерации и pipeline выдаются только скриптом.

## Текущая структура

- `backend/` — FastAPI + pipeline + scheduler; единый API аккаунта (`/auth/*`, JWT `aud=user`)
- `frontend/` — React (TypeScript) лента, детали, модерация, страница аккаунта, контекст `AuthContext`
...
- **«Аккаунт»** → `/account` (`frontend/src/pages/AccountPage.tsx`), сессия в `frontend/src/context/AuthContext.tsx`.
- Модерация и pipeline — те же токены; в шапке ссылка «Модерация» только при `can_moderate`.
- `/login` перенаправляет на `/account` (совместимость со старыми закладками).

**Бэкенд**

- Таблицы: `app_users`, `app_refresh_tokens` (миграция [`backend/alembic/versions/20260604_02_unified_app_users.py`](backend/alembic/versions/20260604_02_unified_app_users.py); перенос из `staff_users` / `reader_users`).
- Маршруты: `backend/app/api/routes/auth.py` — `POST /auth/register`, `/login`, `/refresh`, `/logout`, `GET /auth/me`.
- Редакция: `python -m app.scripts.create_staff_user --email … --password …` или `--grant-only` для существующего читателя.

После обновления кода: **`alembic upgrade head`** (объединение учёток). Пользователям нужно войти заново (новые ключи localStorage и JWT).
