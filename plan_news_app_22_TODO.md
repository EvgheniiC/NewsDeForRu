...
### Продукт: голос (TTS) на карточке

**Статус:** не начато. Добавлено 19.09.2026.

Озвучка карточки через системный `speechSynthesis` / Android TTS. Для русской аудитории в дороге, **без облачного API и без новых лицензий на контент**. Рантайм: **0 €**. Оценка: **2–5 рабочих дней** (MVP 1–2 дня).

Что читать:

- превью: `title` + `subtitle` (сразу, без сети);
- после «Раскрыть»: суть + «простым языком» + «что сделать»;
- не читать: лицензии, URL источника, бейджи.

Не делать в v1: OpenAI/Google/Azure/ElevenLabs TTS (деньги, GDPR, хранение mp3).

#### MVP (Web Speech API) — 1–2 дня

- [ ] Модуль `frontend/src/lib/cardTts.ts`: singleton `speak` / `pause` / `resume` / `stop`; `lang = "ru-RU"`; выбор голоса с `lang.startsWith("ru")`.
- [ ] Длинный текст резать на куски (Android обрезает utterance). Перед стартом `speechSynthesis.cancel()`.
- [ ] Кнопка в `NewsCard` (`news-card-actions`): «Слушать» / «Пауза» / стоп. Старт только по жесту пользователя.
- [ ] Стоп при смене карточки, сворачивании, уходе со страницы / `visibilitychange`.
- [ ] Если русского голоса нет — понятная ошибка (не молчаливый провал); на Android подсказка про языковой пакет TTS.

#### Надёжный Android (если WebView врёт) — +1–2 дня

- [ ] Проверить MVP на реальном устройстве (очередь, пауза, свайп в `TikTokFeed`).
- [ ] При багах WebView: `@capacitor-community/text-to-speech` (нативный `TextToSpeech`), тот же UX кнопки.
- [ ] Не обещать прослушивание с выключенным экраном в v1 (нужен foreground service — отдельный кусок, ещё 2–4 дня).

#### Релиз-полировка — +1 день

- [ ] Скорость 0.9–1.2; авто-следующая карточка только по явному opt-in.
- [ ] Privacy (DE + RU): синтез на устройстве, текст на сторонний TTS-сервер не уходит.
- [ ] Аналитика: событие прослушивания в существующую очередь engagement (после согласия).
- [ ] Unit-тесты модуля (нет голоса / cancel / смена id); смоук на Android APK.

Не смешивать с Play-требованиями 2027 (`TODO_LIST_NEW_UPDATE_GOOGLE_PLAY.md`) и outreach источников.
