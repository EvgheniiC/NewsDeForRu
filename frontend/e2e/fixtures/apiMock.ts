...
export async function installApiMock(
  ...
  await page.route(`${MOCK_API}/**`, async (route: Route) => {
    ...
    if (path === "/users/me/library" && method === "GET") {
      if (!bearerPresent(route)) {
        await fulfillJson(route, { detail: "Not authenticated" }, 401);
        return;
      }
      await fulfillJson(route, { saved: [], read: [] });
      return;
    }

    if (path === "/users/me/library" && method === "PUT") {
      if (!bearerPresent(route)) {
        await fulfillJson(route, { detail: "Not authenticated" }, 401);
        return;
      }
      await fulfillJson(route, { saved_count: 0, read_count: 0, skipped_news_ids: [] });
      return;
    }
